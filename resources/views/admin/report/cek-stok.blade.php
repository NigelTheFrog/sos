...
@section('content')
            ...
            <div class="modal-content">
                <form action="{{route('cek-stok.store')}}" method="POST">
                    ...
                    <div class="modal-header bg-secondary text-white">
                        ...
                        <button type="button" class="btn-close btn-close-white align-middle" onclick="closeLaporanResume(this)"
                        aria-label="Close">
                    </div>
                            ...
                            <div class="col-sm-9">
                                <select class="form-select" style="width: 110%" name="trsidresume">
                                    @foreach ($listNodoc as $nodoc)
                                        <option value="{{ $nodoc->trsid }}">{{ $nodoc->doccsoid }} - {{ $nodoc->startcsodate }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
    ...
    <div class="modal fade text-left" id="modalLaporanCSO" tabindex="-1">
        <div class="modal-dialog modal modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header bg-secondary text-white">
                    <h1 class="modal-title fs-6" id="mdlMoreLabel">Laporan CSO SRM All Material Pipa Industri, Siku, Strep</h1>
                ...
                <div class="modal-body">
                    <div class="form-group row col">
                        <label class="col=sm-e col-form-label">Nomor Dokumen </label>
                        <div class="col-sm-9">
                            <select class="form-select" style="width: 110%" id="trsidlaporan">
                                @foreach ($listNodoc as $nodoc)
                                    <option value="{{ $nodoc->trsid }}">{{ $nodoc->doccsoid }} - {{ $nodoc->startcsodate }}
                                    </option>
                                @endforeach
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary text-white"><i
                        class="fas fa-print pe-2" onclick="printLaporanCsoClik(this)"></i>Preview</button>
                </div>
            </div>
        </div>
    </div>
